...
export interface CommandInfo {
    ...
    isCustom?: boolean;
    os?: ('win32' | 'linux' | 'darwin')[]; // 支持的操作系统
}

// 内置命令数据库
export const builtinCommandDatabase: CommandInfo[] = [
    {
        name: 'ls',
        description: '列出目录内容',
        usage: 'ls [选项] [路径]',
...
// 自定义命令数据库
let customCommandDatabase: CommandInfo[] = [];

// 命令搜索结果缓存
const commandSearchCache = new Map<string, CommandInfo[]>();
const commandInfoCache = new Map<string, CommandInfo | undefined>();
...
// 合并内置命令和自定义命令
export function getCommandDatabase(): CommandInfo[] {
    return [...builtinCommandDatabase, ...customCommandDatabase];
}

// 按类别组织命令
export function getCommandsByCategory(): Record<string, CommandInfo[]> {
...
// 搜索命令
export function searchCommands(prefix: string): CommandInfo[] {
    ...
    const result = allCommands.filter(cmd => 
        cmd.name.toLowerCase().startsWith(cacheKey)
    );
...
// 加载自定义命令
export function loadCustomCommands(commands: CommandInfo[]): void {
    customCommandDatabase = commands;
    clearCaches();
}

// 获取自定义命令
export function getCustomCommands(): CommandInfo[] {
    return customCommandDatabase;
}

// 添加自定义命令
export function addCustomCommand(command: CommandInfo): boolean {
    // 检查命令是否已存在
...
// 更新自定义命令
export function updateCustomCommand(command: CommandInfo): boolean {
...
// 删除自定义命令
export function deleteCustomCommand(commandName: string): boolean {
...
// 导出自定义命令
export function exportCustomCommands(): CommandInfo[] {
    return customCommandDatabase;
}

// 导入自定义命令
export function importCustomCommands(commands: CommandInfo[]): { success: number; failed: number } {
    let success = 0;
    let failed = 0;
