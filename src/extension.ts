...
import { searchCommands, getCommandInfo, CommandInfo, getCommandsByCategory, loadCustomCommands, getCustomCommands, addCustomCommand, updateCustomCommand, deleteCustomCommand, exportCustomCommands, importCustomCommands } from './commandDatabase';
...
interface DirectoryContext {
    ...
    isKubernetesProject: boolean;
}

class CommandAutocomplete {
    private commandUsage: CommandUsage = {};
    private fileCache: Map<string, vscode.CompletionItem[]> = new Map();
    private directoryContextCache: Map<string, DirectoryContext> = new Map();
    private baseCacheTimeout = 5000; // 基础缓存超时时间（毫秒）
    private maxCacheTimeout = 30000; // 最大缓存超时时间（毫秒）
    private minCacheTimeout = 1000; // 最小缓存超时时间（毫秒）
    private context: vscode.ExtensionContext;
    private currentOS: string = process.platform; // 当前操作系统
    ...
    private commandDetailsPanel: vscode.WebviewPanel | undefined;
    private customCommandsPanel: vscode.WebviewPanel | undefined;
     ...
     * @returns 目录上下文信息
     */
    private getDirectoryContext(document: vscode.TextDocument): DirectoryContext {
        ...
        try {
            ...
            isKubernetesProject = files.some((file: string) => file.endsWith('.yaml') || file.endsWith('.yml'));
        } catch {
            // 忽略错误
        }
    ...
    private provideCompletionItems(document: vscode.TextDocument, position: vscode.Position) {
        // 获取当前行的文本
        ...
        if (isFirstToken) {
            ...
            if (commandInfo && commandInfo.options) {
                // 过滤匹配的选项
                const matchingOptions = commandInfo.options.filter(option => 
                    option.flag.toLowerCase().startsWith(currentToken.toLowerCase())
                );
     ...
     * @returns 命令的上下文权重
     */
    private getContextWeight(command: string, context: DirectoryContext): number {
        let weight = 0;
        
        // 根据项目类型调整命令权重
        if (context.isNodeProject) {
            if (command === 'npm' || command === 'yarn') weight += 100;
        }
        
        if (context.isPythonProject) {
            if (command === 'pip') weight += 100;
        }
        
        if (context.isGitRepo) {
            if (command === 'git') weight += 80;
        }
        
        if (context.isDockerProject) {
            if (command === 'docker') weight += 90;
        }
        
        if (context.isKubernetesProject) {
            if (command === 'kubectl') weight += 90;
        }
    ...
    // 排序结果缓存
    private rankedCommandsCache: Map<string, CommandInfo[]> = new Map();

    // 获取排序后的命令列表
    private getRankedCommands(prefix: string, directoryContext: DirectoryContext): CommandInfo[] {
        // 生成缓存键
        ...
        let matchingCommands = searchCommands(prefix);
        ...
        matchingCommands = matchingCommands.filter(cmd => {
            ...
            // 检查当前操作系统是否在命令支持的列表中
            return cmd.os.includes(this.currentOS as 'win32' | 'linux' | 'darwin');
        });
    ...
    // 从存储加载命令使用记录
    private loadCommandUsage() {
        const storedUsage = this.context.globalState.get<CommandUsage>('commandUsage');
        if (storedUsage) {
    ...
    // 保存命令使用记录到存储
    private saveCommandUsage() {
        this.context.globalState.update('commandUsage', this.commandUsage);
    }

    // 更新命令使用记录
    private updateCommandUsage(command: string) {
    ...
    // 获取排序文本
    private getSortText(command: string, directoryContext: DirectoryContext): string {
            ...
            if (parts.length > 0) {
                ...
                if (commandCount > 0) {
                    ...
                    const timeout = Math.min(
                        this.maxCacheTimeout,
                        Math.max(
                            this.minCacheTimeout,
                            this.baseCacheTimeout * (1 + avgUsage * 0.1)
                        )
                    );
                    return timeout;
                }
    ...
    // 获取文件路径补全
    private getFileCompletions(currentToken: string, document: vscode.TextDocument): vscode.CompletionItem[] {
        ...
        try {
            ...
            // 异步读取目录，避免阻塞主线程
            fs.readdir(basePath, (err: any, files: string[]) => {
                if (err) {
                    return;
                }
                
                const fileItems: vscode.CompletionItem[] = [];
                let processed = 0;
    ...
    // 获取环境变量补全
    private getEnvironmentVariableCompletions(currentToken: string): vscode.CompletionItem[] {
        const items: vscode.CompletionItem[] = [];
     ...
     * @returns 命令文档
     */
    private getCommandDocumentation(commandInfo: CommandInfo): vscode.MarkdownString {
        const md = new vscode.MarkdownString();
     ...
     */
    private showCommandDetails(commandName: string) {
        const commandInfo = getCommandInfo(commandName);
        if (!commandInfo) {
        ...
        // 创建新的面板
        this.commandDetailsPanel = vscode.window.createWebviewPanel(
            'commandDetails',
            `${commandName} Command Details`,
     ...
     * @param commandInfo 命令信息
     */
    private updateCommandDetailsPanel(commandInfo: CommandInfo) {
        if (!this.commandDetailsPanel) {
            return;
        }
    ...
    private loadCustomCommands() {
        const storedCommands = this.context.globalState.get<CommandInfo[]>('customCommands');
        if (storedCommands) {
            loadCustomCommands(storedCommands);
        }
    }
    ...
    private manageCustomCommands() {
        ...
        // 创建新的面板
        this.customCommandsPanel = vscode.window.createWebviewPanel(
            'customCommands',
            'Custom Commands',
            vscode.ViewColumn.Beside,
            {
        ...
        // 处理消息
        this.customCommandsPanel.webview.onDidReceiveMessage(
            message => {
    ...
    // 更新自定义命令面板
    private updateCustomCommandsPanel() {
        if (!this.customCommandsPanel) {
            return;
        }
    ...
    // 添加自定义命令
    private addCustomCommand(commandInfo: CommandInfo) {
        const success = addCustomCommand(commandInfo);
        if (success) {
    ...
    // 编辑自定义命令
    private editCustomCommand(commandInfo: CommandInfo) {
        // 这里可以实现更复杂的编辑功能，暂时只做简单提示
        vscode.window.showInformationMessage(`Edit command: ${commandInfo.name}`);
    }

    // 删除自定义命令
    private deleteCustomCommand(commandName: string) {
        const success = deleteCustomCommand(commandName);
        if (success) {
            this.saveCustomCommands();
            this.updateCustomCommandsPanel();
    ...
    // 导出自定义命令
    private exportCustomCommands() {
        const customCommands = exportCustomCommands();
        if (customCommands.length === 0) {
            ...
            if (uris && uris.length > 0) {
                try {
                    ...
                    if (Array.isArray(commands)) {
                        const result = importCustomCommands(commands);
                        this.saveCustomCommands();
